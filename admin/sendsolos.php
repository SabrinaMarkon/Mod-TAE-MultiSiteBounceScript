...
if ($id != "")
{
$resendq = "update solos set sent=\"0\" where id=\"$id\"";
$resendr = mysql_query($resendq);
$query3 = "select * from solos where approved=1 and sent=0 and id=\"$id\" limit 1";
}
if ($id == "")
{
...
while ($line4 = mysql_fetch_array($result4)) {
    ...
    $Message .= "<br><br>--------------------------------------------------------------<br><br>";
    if ($memtypeq == "GOLD") {
        $Message .= "Click the link to receive ".$goldclickearn." credits<br><br>";
    }
    if ($memtypeq == "PRO") {
         $Message .= "Click the link to receive ".$proclickearn." credits<br><br>";
    }
    if (($memtypeq != "GOLD") and ($memtypeq != "PRO")) {
         $Message .= "Click the link to receive ".$freeclickearn." credits<br><br>";
    }
    $Message .= "<a href=\"".$domain."/clicks.php?userid=".$useridq."&seed=".$random."&id=".$id."\" target=\"_blank\">".$domain."/clicks.php?userid=".$useridq."&seed=".$random."&id=".$id."</a>";
    $Message .= ".<br><br>";
    $Message .= "--------------------------------------------------------------<br><br>";
